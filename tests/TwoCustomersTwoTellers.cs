...
public class TwoCustomersTwoTellers
{
    ...
    public void Step04_Customer2Leaves()
    {
        ...
        simulation!.CurrentTime.Should().Be(begin.AddMinutes(4));
        simulation.WaitingCustomers.Should().HaveCount(0);
        simulation.MaxWaitTime.Should().Be(TimeSpan.FromMinutes(0));
        simulation.AvailableTellers.Should().Be(2);
    }

    [Test]
    public void FullSimulation()
    {
        simulation!.Run().Should().Be(TimeSpan.FromMinutes(0));
    }
}